...
const contract_address = "0x4C7cd7F36c67f583cCE975088c813085C39FF5f6"; //Change This!!! 
...
const transfer = async (sender, receiver, amount) => {
  try {
    var account = await web3.eth.getAccounts();//Sender will be account zero on the blockchain (Technically the creator).
    contractDetails.methods.transferTokens(sender, receiver, amount).send({from: account[0]});
